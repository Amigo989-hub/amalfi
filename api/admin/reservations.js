...
module.exports = async (req, res) => {
  ...
  if (!access) return;
  if (req.method === "GET") {
    const { data, error } = await access.admin.from("reservations").select("*").order("created_at", { ascending: false }).limit(200);
    if (error) return res.status(500).json({ error: "Reservierungen konnten nicht geladen werden." });
  ...
  if (req.method === "PATCH") {
    ...
    const { error } = await access.admin.from("reservations").update({ status, updated_at: new Date().toISOString() }).eq("id", id);
